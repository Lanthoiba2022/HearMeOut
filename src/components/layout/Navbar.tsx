...
import { Menu, X, Plus, User, Home, LogIn, LogOut, Moon, Sun } from 'lucide-react';
...
const Navbar = () => {
  ...
  const { user, profile, signOut, autoLogin  } = useAuth();
  ...
  const handleSignIn = () => {
    // For development, automatically log in
    autoLogin();
    setIsMenuOpen(false);
  };
    ...
    <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
      <div className="container flex h-16 items-center justify-between px-4">
        <Link to="/" className="flex items-center space-x-2">
          ...
          <span className="font-semibold text-lg tracking-tight">HearMeOut</span>
        </Link>
